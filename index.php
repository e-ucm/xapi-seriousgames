...
$generator = new ProfileGenerator(PROFILE_FILE);
...
foreach ($terms as $term) {
	...
	$tableBodyRow = <<<EOT
<tbody typeof="xapi:i{$type}" about="{$id}" id="{$prefLabel}">
<tr class="{$tr_class}">
...
private function generateDropdown($terms, $title) {
...
foreach ($terms as $term) {
	...
	$dropdownBody .= "<li><a href=\"#{$name}\">{$name}</a></li>";
}
...
usort($verbs, array('ProfileGenerator', 'sort_by_label'));
usort($activities, array('ProfileGenerator', 'sort_by_label'));
usort($extensions, array('ProfileGenerator', 'sort_by_label'));
...
$navbar = '';
$navbar .= $this->generateDropdown($verbs, 'Verbs');
$navbar .= $this->generateDropdown($activities, 'Activity Types');
$navbar .= $this->generateDropdown($extensions, 'Extensions');
